...
class ArticleImageSourceExtractor(HTMLParser):
    def __init__(self):
        super().__init__()
...
def create_article(article_dto: ArticleDTO, cur_user: User) -> Article:
    ...
    logger.info("New article created", extra={"context": {"article_id": article_id}})
...
def create_article_comment(article: Article, article_comment_dto: ArticleCommentDTO, cur_user: User,
    ...
    logger.info("New comment added", extra={"context": {"article_id": article.id, "comment_id": comment_id}})
...
def update_article_status(article: Article, update_article_status_dto: UpdateArticleStatusDTO, cur_user: User,
    ...
    logger.info("Article status changed", extra={"context": {"article_id": article.id, "status": status}})
...
def find_static_image_filename(html_content: str) -> str | None:
    ...
    parser = ArticleImageSourceExtractor()
    parser.feed(html_content)
    parser.close()
